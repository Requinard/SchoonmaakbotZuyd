...
//Alleen herbruikbaar als functie wordt aangepast
bool LinkedListDeepCopy(LinkedListHeader *original, LinkedListHeader *copy)
{
	//Kopie opnieuw initialiseren
	LinkedListContruct(copy);

	//Deep copy
	int cnt = 1;
	while(original->count != copy->count)
	{
		LinkedListNode *node = (LinkedListNode*)malloc(sizeof(LinkedListNode));
		node->data = LinkedListItem(original, cnt)->data;
		LinkedListAddRear(copy, node);
		cnt++;
	}
